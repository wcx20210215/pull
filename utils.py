...
3. **图表生成指导**：（让数据可视化更精彩）📊
   - 当用户询问分布、比较、排名等问题时，优先生成柱状图
   - 当用户询问趋势、变化、时间序列时，优先生成折线图
   - 当用户询问占比、比例、构成时，优先生成饼图
   - 图表数据必须是数值类型，确保columns和data数组长度一致
   - 图表标题应该简洁明了，反映数据的核心信息

4. **格式校验要求**：（别担心，我会小心谨慎，确保万无一失）🛡️
   - 字符串值必须使用英文双引号（别问我为什么，这是规矩）🤔
   - 数值类型不得添加引号（数字就是数字，不需要伪装）🚫
   - 确保数组闭合无遗漏（我可不想漏掉任何重要信息）✅
   - 图表数据中的columns必须是字符串数组，data必须是数值数组

   错误案例：（反面教材，千万别学）💩
   {'columns':['Product', 'Sales'], data:[[A001, 200]]}
   {"bar":{"columns": ["A", "B"], "data":["35", "42"]}} // 数据不应该是字符串

   正确案例：（这才是正确的打开方式）👍
   {"columns":["product", "sales"], "data":[["A001", 200]]}
   {"bar":{"columns": ["A", "B"], "data":[35, 42]}} // 数据必须是数值
...
def dataframe_agent(df, query, model_choice="gpt-4o-mini", use_cache=True, 
    ...
    try:
        ...
        # 调试信息：打印原始响应
        print(f"AI原始响应: {response['output']}")
        ...
        for chart_type in ["bar", "line", "pie"]:
            if chart_type in result:
                ...
                if not isinstance(chart_data, dict) or "columns" not in chart_data or "data" not in chart_data:
                    print(f"警告: {chart_type}图表数据格式不正确")
                    del result[chart_type]
                elif len(chart_data["columns"]) != len(chart_data["data"]):
                    print(f"警告: {chart_type}图表的columns和data长度不匹配")
                    del result[chart_type]
                else:
                    print(f"✅ {chart_type}图表数据格式正确")
